...
adata <- read_h5ad(path = "crc-to-liver-mets.h5ad", mode = "r+")
adata$obs |> dplyr::glimpse() # Good
adata$var |> dplyr::glimpse() # Annoying, but we will fix this below
adata$X |> abind::acorn() # Good


## HVG selection Round 1 -------------------------------------------------------
# -- We will select 2500 genes out of the 5000.
# -- We will use NEBULA to identify the genes.
# -- Afterwards, we will do processing iteration 1 (see next script)
...
# dir.create("05_hvg_selection_iter1_outs")
# qs2::qs_save(object = nfit, file = "05_hvg_selection_iter1_outs/hvg_nfit.qs2")
nfit <- qs2::qs_read(file = "05_hvg_selection_iter1_outs/hvg_nfit.qs2")
...
# pdf("05_hvg_selection_iter1_outs/2500_HVGs.pdf", width = 6, height = 6)
plot(x = hvg_data$mean_expr, hvg_data$overdispersion, pch = 16, cex = 0.5,
     col = ifelse(hvg_data$gene %in% hvgs, yes = "red", no = "black"))
# dev.off()

# pdf("05_hvg_selection_iter1_outs/mean-variance_trend.pdf", width = 6, height = 6)
